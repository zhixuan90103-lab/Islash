...
**落地规则、网格、外观以 [SLASH-DESIGN.md](./SLASH-DESIGN.md) 为准。** 下文「BoxGeometry 三角剖分 / ConvexObjectBreaker」是调研当时的方案，**不要再落地**。当前是 **2D 轮廓切开 + 竖直挤出 + 凸包半平面内收倒角**（不是锥台，切的不是 3D 网格）。仍不做关卡、胜负、分数、其他手势族。
...
> 本节是调研当时的对照。落地规则见 [SLASH-INTENT.md](./SLASH-INTENT.md)：微段判穿、同一按住可多刀；**已交刀的有向直线在走廊内不再切留下的切面**。下面「和本仓库的差距」里的 `cutDone` 等已过时。
