...
1. 轮廓沿 Z **竖直挤出**（侧面不斜）。
2. 只在朝相机那一圈、**按边**做等宽倒角：边内移 `frontInset`，Z 向同一宽度 → 约 45°。
3. 背面 = 完整轮廓。正面由各边内沿拼成。

`createFrustumGeometry` 是历史名，实现已是 `createWoodSolid`，**不是锥台**。

### 一刀怎么切

1. 刀线投到板面 XY，`splitConvexPolygon` 得到两块凸多边形。
2. 删旧 mesh。两块各自再跑同一套挤出 + 按边倒角。
3. 面积 × 厚度大的留下（static），小的飞出（dynamic）。

### 倒角（设计，不是补丁）

倒角是**边上的局部特征**，不是「整块内收，失败就整圈降档」。

| 步骤 | 规则 |
|------|------|
| 轮廓 | `cleanConvex` 去重、过短边、共线点。这是切开后的真源。 |
| 每条边 | 沿内法线平移 `frontInset`，再用**其余偏移半平面裁这段**。内轮廓一定是简单凸多边形。 |
| 锐角 | 两条宽 d 的倒角带在尖楔里会相交。该角的 miter 会落在别的半平面外 → **丢掉这个内顶点**，用一条内沿补上。切边仍是原直边挤出，切面平整。 |
| 整块没棱 | 仅当 `inset ≥ 轮廓最小高`，或裁完剩下不足 3 条内边。 |
| 法线 | 每个三角形自己的面法线；材质 `flatShading`。 |

文件：`woodProfile.ts`（轮廓/切开）、`woodChamfer.ts`（按边倒角 + 挤出）、`wood.ts`（生成/重置）。
...
- 大块擦过顶点留下短边：外圈棱还在，不会整圈变平。再切一刀也不该「修好」。
- 锐角：只有那个角被截平，没切到的边宽度不变。
- 细尖碎片：可以没有倒角，但不能穿面。

## 木头尺寸

- `WOOD_SHAPE`：设计形体 **0.7 × 2 × 0.15**（板，不是正方体）。
- `WOOD_SHAPE.frontInset`：正面倒角宽度，默认 **0.028**（XY 与 Z 相同）。
- `WOOD_SHAPE.miterLimit`：默认 **2**。只决定角用 miter 还是截断，失败不会改其它边的 inset。
