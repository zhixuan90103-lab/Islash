...
近期已落地（参数以 `design.ts` 为准）：切缝深度 `max(minChord, 短边×hullChordRatio)`（4px / 4%）；A 锁在板上局部 XY，屏上位置跟板走；入边用刀尖附近最近一次出板（约 100px），帮助锁离刀尖最近的边。余势 `slashFollow.ts`：刀尖还在刚切开的块上，或仍顺着缝，算同一划的尾巴。滑入中途切开只继续带留下块。夹缝 `#b1591a` α0.65；划痕 `life` 0.16、`maxLen` 220。乱划（板内路程 / A→出点 > `pathChordMax` 1.7）不算切开：夹缝收回、无刀光、镜头轻推再回。同时一把有效刀。已回滚：出板才放行、加宽走廊当下一刀开关、切开面永禁当入边。
...
1. **一刀成立**：进出落在**两条不同凸包边上**，且切缝够深。深度 = `max(minChord, hullChordRatio × 投影包围盒短边)`（`throughThreshold`），默认 **4px** 与 **4%**。不要再套一层 `max(4, …)`。同一条边蹭不算。近共面邻边只要仍是两条凸包边且弦够深就能切。终点帮助：对准青线且行程 ≥ T(速度) 时可补出点；最慢须 100% 真出边。锁 A 后板内路程 / A→出点直线 > `START.pathChordMax`（默认 1.7）不算切开。未切开就出板后再进是新刀，不必抬手。
...
7d. **进场**：新板从画面上方滑入到中心（`CUT.enterDur`，ease-out）后停下。不持续下落、不出下边。滑入中途切开：只继续带留下块，飞出块不再被拽回同一条 Y。
...
9. **不伪造**「重的一侧向下」的额外力矩。
10. **触控**走 `clientToDesign`；letterbox 外忽略。调试面板 `stopPropagation`，不抢刀。可同时按下最多 `START.maxStrokes`（3）指，**有效刀只有一把**：已锁 A 的那指，否则划得更长（已出刃优先）。误触按着不动不挡后面真滑的。锁 A 后不换人直到抬起。刀痕只画有效刀。
